...
_text = {
    ...
    "cloptt" : ("A grid based prediction is generated by taking the clusters in order, and assigning risk "
        + "to a grid cell if the centre of the grid is in the cluster disk.  As such, very small disks may "
        + "give rise to very small grid-based disks.  Here we can expand disks to a larger radius but so that "
        + "the contained events does not changed.  This is likely only to make a difference if you have "
        + "selected a 'grid' option in the 'Quantise data' panel above." ),
    "clop1" : "Use as is",
...
class STScanView(tk.Frame):
    ...
    def add_widgets(self, frame):
        ...
        tooltips.ToolTipYellow(subframe, _text["cloptt"])
